...
class CertificateBase(models.Model):
        ...
        if not PKI_ENABLE_EMAIL:
            ...
            if self.email:
                return format_html('<a href="%s">%s</a>' % (reverse(
                    'pki:email',
                    kwargs={'model': self.__class__.__name__.lower(), 'id': self.pk}),
                    get_pki_icon_html(
                        "mail--arrow.png",
    ...
    def download_link_zip(self):
        ...
        if ((self.active and self.__class__.__name__.lower() == 'certificateauthority' and self.public) or 
            ...
            return format_html('<a href="%s">%s</a>' % (reverse('pki:download', kwargs={
                                                                            'model': self.__class__.__name__.lower(),
                                                                            'id': self.pk,
                                                                            'ext':'zip'
                                                                        }
    ...
    def Parent_link(self):
        ...
        if self.parent:
            ...
            return format_html('<a href="%s">self-signed</a>' % (reverse(
                'admin:pki_%s_change' % self.__class__.__name__.lower(),
                args=(self.pk,))))
...
class CertificateAuthority(CertificateBase):
    ...
    parent_passphrase = models.CharField(
        ...
        null=True,
        blank=True,
        help_text="Leave empty if this is a top-level CA"
    )
    ...
    def Tree_link(self):
        ...
        if PKI_ENABLE_GRAPHVIZ:
            return  format_html('<a href="%s" target="_blank">%s</a>' % (reverse('pki:tree', kwargs={'id': self.pk}),
                                                            get_pki_icon_html(
                                                                "tree.png",
                                                                "Show CA tree",
    ...
    def Child_certs(self):
        ...
        if not self.is_edge_ca():
            ...
            return  format_html("<a href=\"%s\" target=\"_blank\">%s</a>" % ('?'.join([
                                                                        reverse('admin:pki_certificate_changelist'),
...
class Certificate(CertificateBase):
    ...
    def download_link_ovpn(self):
        ...
        if self.active and self.extension.name.find('client') !=-1 and self.__class__.__name__.lower() == 'certificate':
            return format_html('<a href="%s">%s</a>' % (
                reverse('pki:download', kwargs={
                                        'model': self.__class__.__name__.lower(),
                                        'id': self.pk,
                                        'ext': 'ovpn'
                                    }
        ...
        if self.pk:
            if self.action in ('update', 'revoke', 'renew'):
                ...
                if self.action == 'revoke':
                    ...
                    if prev.country != self.country: c_list.append('Updated country to "%s"' % self.country)
                    if prev.locality != self.locality: c_list.append('Updated locality to "%s"' % self.locality)
                    if prev.organization != self.organization: c_list.append(
                        'Updated organization to "%s"' % self.organization
                    )
                    if prev.email != self.email: c_list.append('Updated email to "%s"' % self.email)
                    if prev.OU != self.OU: c_list.append('Updated OU to "%s"' % self.OU)
...
class x509Extension(models.Model):
    ...
    subject_key_identifier = models.CharField(
        ...
        default="hash",
        verbose_name="subjectKeyIdentifier"
    )
