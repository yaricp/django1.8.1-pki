...
class CertificateBaseAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        urls = super().get_urls()
        my_urls = [
            path('refresh_metadata/',
                self.admin_site.admin_view(self.refresh_metadata),
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name == "parent":
            kwargs["queryset"] = CertificateAuthority.objects.filter(
                extension__basic_constraints__contains="CA:TRUE",
                active=True).exclude(extension__basic_constraints__contains="pathlen:0")
