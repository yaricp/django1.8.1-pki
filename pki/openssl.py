...
from .settings import PKI_OPENSSL_BIN, PKI_OPENSSL_CONF, PKI_DIR, PKI_OPENSSL_TEMPLATE, \
                         PKI_SELF_SIGNED_SERIAL, PKI_CA_NAME_BLACKLIST, PKI_DEFAULT_ALGORITHM
...
def refresh_pki_metadata(ca_list):
    ...
    try:
        ...
        conf = render_to_string(PKI_OPENSSL_TEMPLATE, {
            'home_dir': PKI_DIR,
            'ca_list': ca_list,
...
class Openssl:
    ...
    def generate_self_signed_cert(self):
        ...
        logger.info("Generating new self-signed certificate (CN=%s, x509 extension=%s)" % (
            self.i.common_name,
            self.i.extension
        ))
