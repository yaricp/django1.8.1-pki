...
def files_for_object(obj):
    ...
    files = {
                'chain': {
                    'path': os.path.join(ca_dir, '%s-chain.cert.pem' % chain),
                    'name': '%s-chain.cert.pem' % chain,
                    },
                'crl': {
                    'path': os.path.join(ca_dir, 'crl', '%s.crl.pem' % chain),
                    'name': '%s.crl.pem' % chain,
                    },
                'pem': {
                    'path': os.path.join(ca_dir, 'certs', '%s.cert.pem' % c_name),
                    'name': '%s.cert.pem' % c_name,
                    },
                'csr': {
                    'path': os.path.join(ca_dir, 'certs', '%s.csr.pem' % c_name),
                    'name': '%s.csr.pem' % c_name,
                    },
                'der': {
                    'path': os.path.join(ca_dir, 'certs', '%s.cert.der' % c_name),
                    'name': '%s.cert.der' % c_name,
                    },
                'pkcs12': {
                    'path': os.path.join(ca_dir, 'certs', '%s.cert.p12' % c_name),
                    'name': '%s.cert.p12' % c_name,
                    },
                'key': {
                    'path': os.path.join(ca_dir, key_loc, '%s.key.pem' % c_name),
...
def build_delete_item(obj):
    ...
    return "<ul><li>Serial: %s</li><li>Subject: %s</li><li>Parent: %s</li><li>Description: %s</li>"\
