...
from .helper import *
from .models import Certificate, CertificateAuthority, PkiChangelog, x509Extension
from .settings import PKI_DIR, PKI_ENABLE_EMAIL, PKI_ENABLE_GRAPHVIZ
...
class CertificateBaseModelTestCases(TestCase):
    ...
    def setUp(self):
        self.obj = CertificateAuthority(
            ...
            state="Bavaria",
            locality="Munich",
            organization="Bozo Clown Inc.",
            OU="IT",
            email="a@b.com",
            valid_days=1000,
            key_length=1024,
            expiry_date=datetime.datetime(2021, 01, 01, 20, 00, 00).date(),
            created=datetime.datetime(2011, 01, 01, 20, 00, 00),
            revoked=datetime.datetime(2011, 01, 01, 20, 00, 00),
            active=None,
            serial=None,
            ca_chain=None,
            der_encoded=False,
...
class OpensslTestCases(TestCase):
    ...
    def setUp(self):
        self.ca = CertificateAuthority(
            ...
            parent=None,
            passphrase="1234567890",
            extension=x509Extension.objects.get(pk=1),
        )
...
class HelperTestCase(TestCase):
    ...
    def test_subject_for_object(self):
        self.assertEqual(
            subject_for_object(self.obj),
            "/CN=%s/C=%s/ST=%s/localityName=%s/O=%s/organizationalUnitName=%s/emailAddress=%s"
            % (
                self.obj.common_name,
                self.obj.country,
                self.obj.state,
                self.obj.locality,
...
def CreateCaChain():
    ...
    CertificateAuthority(
        ...
        parent_passphrase="1234567890",
        passphrase="1234567890",
        extension=x509Extension.objects.get(pk=2),
    ).save()
...
class CertificateAuthorityTestCase(TestCase):
    ...
    def test_HistoryUpdated(self):
        ...
        self.rca.save()
        self.assertEqual(
            PkiChangelog.objects.get(
                model_id=ContentType.objects.get_for_model(self.rca).pk,
                object_id=self.rca.pk,
                changes='Updated description to "UNIT_TEST_UPDATE"',
            ).changes,
...
class CertificateTestCase(TestCase):
    ...
    def setUp(self):
        ...
        Certificate(
            common_name="Server Edge Certificate",
            name="Server_Edge_Certificate",
            description="unit test server edge certificate",
            country="DE",
            state="Bavaria",
            locality="Munich",
            organization="Bozo Clown Inc.",
            ...
            ca_chain=None,
            der_encoded=False,
            pkcs12_encoded=False,
            pkcs12_passphrase=None,
            parent=self.eca,
            parent_passphrase="1234567890",
            passphrase=None,
            extension=x509Extension.objects.get(pk=3),
            subjaltname="IP:1.2.3.4, DNS:www1.company.com",
        ).save()

        Certificate(
            common_name="User Edge Certificate",
            name="User_Edge_Certificate",
            description="unit test user edge certificate",
            country="DE",
            state="Bavaria",
            locality="Munich",
            organization="Bozo Clown Inc.",
            ...
            parent_passphrase="1234567890",
            passphrase=None,
            extension=x509Extension.objects.get(pk=4),
            crl_dpoints="URI:https://ca.company.com/ca.crl",
        ).save()
    ...
    def test_HistoryUpdated(self):
        ...
        self.assertEqual(
            PkiChangelog.objects.get(
                model_id=ContentType.objects.get_for_model(self.srv).pk,
                object_id=self.srv.pk,
                changes='Updated description to "UNIT_TEST_UPDATE"',
            ).changes,
            'Updated description to "UNIT_TEST_UPDATE"',
        )
    ...
    def test_ParseRawCertificate(self):
        ...
        self.assertTrue(
            re.search("X509v3 Subject Alternative Name:\s*\n\s*IP Address:1.2.3.4, DNS:www1.company.com", c)
        )
...
class HttpClientTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.post_data_eca = {
            ...
            "parent_passphrase": "1234567890",
            "policy": "policy_anything",
            "extension": x509Extension.objects.get(name="v3_edge_ca").pk,
        }
    ...
    def test_DeleteEdgeCertificateAuthority(self):
        ...
        eca_ssl = openssl.Openssl(eca_obj)
        r = self.c.post(
            "/admin/pki/certificateauthority/3/delete/",
            {"_model": "certificateauthority", "_id": 3, "passphrase": "1234567890"},
            follow=True,
        )
    ...
    def test_DeleteRootCertificateAuthority(self):
        ...
        r = self.c.post(
            "/admin/pki/certificateauthority/1/delete/",
            {"_model": "certificateauthority", "_id": 1, "passphrase": "1234567890"},
            follow=True,
        )
        self.assertContains(r, "was deleted successfully")
        self.failUnlessEqual(r.status_code, 200)
