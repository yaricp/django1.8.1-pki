...
from .openssl import Openssl, md5_constructor, refresh_pki_metadata
from .settings import STATIC_URL, PKI_DEFAULT_COUNTRY, PKI_ENABLE_GRAPHVIZ, \
                         PKI_ENABLE_EMAIL, PKI_PASSPHRASE_MIN_LENGTH, PKI_DEFAULT_KEY_LENGTH, \
                         PKI_DEFAULT_OU, PKI_DEFAULT_STATE, PKI_DEFAULT_LOCALITY, \
                         PKI_DEFAULT_EMAIL, PKI_DEFAULT_VALID_DAYS
...
class CertificateBase(models.Model):
    ...
    valid_days   = models.IntegerField(validators=[MinValueValidator(1)], 
                                                            default=PKI_DEFAULT_VALID_DAYS)
    ...
    crl_dpoints  = models.CharField(max_length=255, verbose_name='CRL Distribution Points', 
                                                        null=True, blank=True, validators=[validate_crl_dp], \
                                                        help_text='Comma seperated list of URI elements.'\
                                                                            ' Example: URI:http://ca.local/ca.crl,...')
    ...
    def Revocation_date(self):
        ...
        if self.revoked:
            ...
            out = ''
        return out 
        #self.revoked.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def Chain_link(self):
        ...
        if PKI_ENABLE_GRAPHVIZ:
            return '<a href="%s" target="_blank">%s</a>' % (urlresolvers.reverse('chain', kwargs={'model': self.__class__.__name__.lower(), 'id': self.pk}), \
    ...
    def Email_link(self):
        ...
        if not PKI_ENABLE_EMAIL:
            ...
            if self.email:
                return '<a href="%s">%s</a>' % (urlresolvers.reverse('email', kwargs={'model': self.__class__.__name__.lower(), 'id': self.pk}), \
    ...
    def download_link_zip(self):
        ...
        if ((self.active and self.__class__.__name__.lower() == 'certificateauthority' and self.public) or 
            ...
            return '<a href="%s">%s</a>' % (urlresolvers.reverse('download', kwargs={'model': self.__class__.__name__.lower(), 
                                                                                                                                                    'id': self.pk, 
                                                                                                                                                    'ext':'zip'}), \
                                            get_pki_icon_html("drive-download.png", "Download certificate zip",
    ...
    def download_link_crt(self):
        ...
        if ((self.active and self.__class__.__name__.lower() == 'certificateauthority' and self.public) or 
            ...
            return '<a href="%s">%s</a>' % (urlresolvers.reverse('download', kwargs={'model': self.__class__.__name__.lower(), 
                                                                                                                                            'id': self.pk, 
                                                                                                                                            'ext':'crt'}), \
                                            get_pki_icon_html("drive-download.png", "Download certificate .crt",
                                                                            id="download_link_%d" % self.pk))
        else:
            return get_pki_icon_html("drive-download_bw.png", "Certificate is revoked. Disabled", id="download_link_%d" % self.pk)
    ...
    def download_link_p12(self):
        ...
        if ((self.active and self.__class__.__name__.lower() == 'certificateauthority' and self.public) or 
            ...
            return '<a href="%s">%s</a>' % (urlresolvers.reverse('download', kwargs={'model': self.__class__.__name__.lower(), 
                                                                                                                                            'id': self.pk, 
                                                                                                                                            'ext':'p12'}), \
                                            get_pki_icon_html("drive-download.png", "Download certificate .pkcs12",
                                                                            id="download_link_%d" % self.pk))
        else:
            return get_pki_icon_html("drive-download_bw.png", "Certificate is revoked. Disabled", id="download_link_%d" % self.pk)
    ...
    def download_link_crl(self):
        ...
        if ((self.active and self.__class__.__name__.lower() == 'certificateauthority' and self.public) or 
            (self.active and self.__class__.__name__.lower() == 'certificate')):
            return '<a href="%s">%s</a>' % (urlresolvers.reverse('download', 
                                                                                                        kwargs={'model': self.__class__.__name__.lower(), 
                                                                                                                        'id': self.pk, 
                                                                                                                        'ext':'crl'}), \
                                                                    get_pki_icon_html("drive-download.png", 
                                                                                                "Download certificate .crl",
                                                                                                id="download_link_%d" % self.pk))
        else:
            return get_pki_icon_html("drive-download_bw.png", "Certificate is revoked or disabled or not public", id="download_link_%d" % self.pk)
    ...
    def CA_Clock(self):
        """"""
        return '<div id="clock_container"><img src="%spki/img/clock-frame.png" style="margin-right:5px"/><span id="clock"></span></div>' % STATIC_URL
    ...
    def Update_Changelog(self, obj, user, action, changes):
        ...
        PkiChangelog(model_id=ContentType.objects.get_for_model(obj).pk, object_id=obj.pk, action=action, 
                                user=user, changes="; ".join(changes)).save()
...
class CertificateAuthority(CertificateBase):
    ...
    policy            = models.CharField(max_length=50, choices=POLICY, default='policy_anything', 
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk:
            if self.action in ('update', 'revoke', 'renew'):
                ...
                ## Update public. This is always allowed
    ...
    def Tree_link(self):
        ...
        if PKI_ENABLE_GRAPHVIZ:
            return '<a href="%s" target="_blank">%s</a>' % (urlresolvers.reverse('tree', kwargs={'id': self.pk}), \
...
class Certificate(CertificateBase):
    ...
    owner = models.ForeignKey(User, 
                                                    verbose_name='Owner of certificate')
