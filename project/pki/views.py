...
from .settings import PKI_LOG, STATIC_URL, PKI_ENABLE_GRAPHVIZ, PKI_ENABLE_EMAIL
...
from .helper import (chain_recursion, build_delete_item, generate_temp_file, 
                                    build_zip_for_object, get_full_path_file)
...
def home_page(request):
    ...
    return render_to_response('home.html', {'certs':certs, 
                                                                            'personal_certs':personal_certs, 
                                                                            'request':request})
...
def pki_download(request, model, id, ext):
    ...
    if model == "certificateauthority":
        ...
        if request.user.is_authenticated:
            ...
            return HttpResponseRedirect(urlresolvers.reverse('accounts:login'))
    else:
    ...
    if os.path.exists(file):
        ...
        response['Content-Disposition'] = 'attachment; filename="%s.%s"' % (c.name, ext)
