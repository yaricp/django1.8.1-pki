...
from .settings import PKI_DIR, PKI_LOG, PKI_LOGLEVEL, JQUERY_URL
...
class CertificateBaseAdmin(admin.ModelAdmin):
    ...
    class Media:
        js  = ( JQUERY_URL, 'js/jquery.tipsy.js', 'js/pki_admin.min.js', )
        css = { 'screen': ( 'css/pki.css', 'css/tipsy.css', ), }
...
class Certificate_Authority_Admin(CertificateBaseAdmin):
    ...
    list_display       = ( 'id', 'common_name', 'public','Serial_align_right', 
                                'Valid_center', 'Chain_link', 'Tree_link', 'Parent_link',
                                'Expiry_date', 'Description', 'Creation_date', 'Revocation_date', 
                                'Child_certs', 'download_link_zip', 'download_link_crt','download_link_crl',
                                'Email_link', )
    ...
    readonly_fields    = ( 'Expiry_date', 'Creation_date', 'Revocation_date', 'serial', 
                                    'Chain', 'Certificate_Dump', 'CA_Clock', 'State', )
    fieldsets          = ( ( 'Define action',    { 'fields': ( 'action', ), }, ),
                           ...
                           ( 'Certificate',      { 'fields': ( 'State', 'common_name', 'name', 'country', 'public',
                                                                                    'state', 'locality', 'organization',
                                                                                    'OU', 'email', 'key_length', 'valid_days',
                                                                                    'extension', 'passphrase', 'passphrase_verify',
                                                                                    'serial', 'Expiry_date', 'Creation_date', 
                                                                                    'Revocation_date',
                                                             ),
                                                   'classes': [ 'wide', ],
                                                 },
                           ...
                           ( 'Certificate signing', { 'fields': ( 'CA_Clock', 'Chain', 'parent', 
                                                                                            'parent_passphrase', 'crl_dpoints', 'policy', ),
                                                      'classes': [ 'wide', ],
                                                    },
                           ),
                         )
...
class Certificate_Admin(CertificateBaseAdmin):
    ...
    list_display       = ( 'id', 'common_name','owner',  'Serial_align_right', 'Valid_center', 'Chain_link', 
                                    'Parent_link','Expiry_date', 'Description', 'Creation_date', 'Revocation_date', 
                                    'download_link_zip', 'download_link_crt', 'download_link_p12', 'Email_link' )
    list_display_links = ( 'common_name', )
    radio_fields       = { "action": admin.VERTICAL }
    list_filter        = ( 'parent', 'active', 'extension', 'owner')
    search_fields      = [ 'name', 'description' ]
    date_hierarchy     = 'created'
    readonly_fields    = ( 'Expiry_date', 'Creation_date', 'Revocation_date', 'serial', 'Chain', 
                                        'Certificate_Dump', 'CA_Clock', 'State', )
    fieldsets          = ( ( 'Define action',   { 'fields': ( 'action', ) } ),
                           ( 'Documentation',   { 'fields': ( 'description', ),
                                                  'classes': [ 'wide', ],
                                                },
                           ...
                           ( 'Certificate',     { 'fields': ( 'State', 'common_name', 'name','owner', 'country', 'state', 
                                                                                    'locality', 'organization','OU', 'email', 'key_length',
                                                                                    'valid_days', 'extension','passphrase', 
                                                                                    'passphrase_verify', 'serial', 'Expiry_date', 
                                                                                    'Creation_date','Revocation_date',
                                                            ),
                                                  'classes': [ 'wide', ],
                                                },
                           ),
                           ...
                           ( 'Encoding options', { 'fields': ( 'der_encoded', 'pkcs12_encoded', 
                                                                                    'pkcs12_passphrase', 'pkcs12_passphrase_verify', ),
                                                   'classes': [ 'wide', ],
                                                 },
                           ),
                           ( 'Certificate signing', { 'fields': ( 'CA_Clock', 'Chain', 'parent', 
                                                                                                'parent_passphrase', 'crl_dpoints', ),
                                                      'classes': [ 'wide', ],
                                                    },
                           ),
                         )
